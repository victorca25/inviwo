...
mat2 Interpolator::sampleJacobian(const Volume* vol, const vec2& position)
{
    auto vr = vol->getRepresentation<VolumeRAM>();
    auto dims = vr->getDimensions();
    mat2 jacobian;
    ...
    {
        vec2 lower = position;
        lower[dim] = std::max(0.0f, position[dim] - 0.5f);
        vec2 higher = position;
        higher[dim] = std::min(float(dims[dim] - 1), position[dim] + 0.5f);

        float dist = higher[dim] - lower[dim];
        vec2 gradient = sampleFromField(vol, higher) - sampleFromField(vol, lower);
        jacobian[dim] = dist > 1e-5 ? gradient / dist : vec2(0.0f);
    }
